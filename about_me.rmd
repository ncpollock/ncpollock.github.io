...
I'm an effective IT leader with extensive technical expertise and a passion for generating value, solving problems, and supporting people and teams. I'm also excited about Data: database architecture, warehousing, machine learning and AI, transaction processing, ETL/ELT, analyzing, reporting, dashboarding, statistics, research, and everything in between. Since 2017, R and SQL have been my go-to tools. However, I have experience with a variety of software, programming languages, and IT project types. You can learn more about my work via the following resources:

<div class="list-group">
<a href="https://oakland0-my.sharepoint.com/:b:/g/personal/noahpollock_oakland_edu/IQB7S16yaqnoSZjh-mmIBamkAXp4yeEtw3K5lUifPrPRFgM?e=BO4VQK" class="list-group-item" target="_blank">
...
I enjoy teaching new skills to my daughter, cycling, running, skiing, problem solving, board-games / cards, DIY, woodworking, power tools, and 3D printing.
...
<div class="panel-body">
  ...
  <p class="caption">Halfway through cycling a century (100 miles) with my wife for the Make-A-Wish Wish-A-Mile 100 fundraiser, 2019.<br></p>
  </div>
  </div>
  ...
  <p class="caption">A business card holder I designed. Mid-print at the Oakland University Makerspace before I invested in my own machine.</p>
  </div>
  </div>
  ...
  <p class="caption">Camping with my wife and dogs. Despite being effective guard dogs, they cower in sheer terror at waves crashing on the shore.</p>
  </div>
  </div>
...
Below are some highlights from my Resume in a dashboard-esque format.
...
pdata <- cv_df %>%
  ...
  mutate(date_end = if_else(is.na(date_end),Sys.Date(),date_end),
         ...
         , Years = round(time_length(interval(as.Date(date_start),as.Date(date_end)),'years'),1))
...
lapply(unique(pdata$category),function(skill_cat){
  ...
  pdata %>%
    ...
    hchart("bar",hcaes(x="experience",y="Years"),color=color_select) %>%
    hc_add_theme(thm) %>%
    hc_title(text = skill_cat) %>%
      hc_tooltip(formatter = JS("function(){
                            return (this.point.name + '<br>' + this.y + ' Year(s)')
                            }")
  ) %>%
    hc_xAxis(title = NA)
}) %>%
...
```
  ...
  <div class="col-md-3">
  <div class="well">
<p style="text-align:center;padding: 8px 0; height:300px;font-size: x-large; background-color: #ecf0f1;"><strong>Select Conference Presentations
<br><br>
